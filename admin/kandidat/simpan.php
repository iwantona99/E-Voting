      ...
      if (is_uploaded_file($foto['tmp_name'])) { //cek apakah ada file yang di upload
         ...
         if (!in_array(($tipe),$ext)){ //cek ekstensi file
            ...
            if (move_uploaded_file($foto['tmp_name'],$dir.$newName)) {
               ...
               $sql = $con->prepare("INSERT INTO t_kandidat(nama_calon, alamat, foto, judull, visi, periode) VALUES(?,?, ?, ?, ?, ?)") or die($con->error);
               $sql->bind_param('ssssss', $nama, $alamat, $newName, $judull , $visi, $periode);
               $sql->execute();
			   $sql = $con->prepare("INSERT INTO histori(nama_calon, alamat, foto, judull, visi, periode) VALUES(?,?,?, ?, ?, ?)") or die($con->error);
               $sql->bind_param('ssssss', $nama, $alamat, $newName, $judull , $visi, $periode);
               $sql->execute();
