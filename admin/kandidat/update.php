   ...
   if (is_uploaded_file($foto['tmp_name'])) {
      ...
      if (!in_array(($tipe),$ext)){ //cek ekstensi file
         ...
         if (move_uploaded_file($foto['tmp_name'],$dir.$newName)) {
            ...
            $sql = $con->prepare("UPDATE t_kandidat SET nama_calon = ? , foto = ?, judull = ?, visi = ? WHERE id_kandidat = ?") or die($con->error);
            $sql->bind_param('sssss', $nama, $newName,$judull, $visi, $id);
            $sql->execute();
			$sql = $con->prepare("UPDATE histori SET nama_calon = ? , foto = ?, judull = ?, visi = ? WHERE id_kandidat = ?") or die($con->error);
            $sql->bind_param('sssss', $nama, $newName,$judull, $visi, $id);
            $sql->execute();
      ...
      $sql->execute();
	  $sql = $con->prepare("UPDATE histori SET nama_calon = ?, visi = ?, misi = ? WHERE id_kandidat = ?") or die($con->error);
      $sql->bind_param('ssss', $nama, $visi, $misi, $id);
      $sql->execute();
