      ...
      if (is_uploaded_file($foto['tmp_name'])) { //cek apakah ada file yang di upload
         ...
         if (!in_array(($tipe),$ext)){ //cek ekstensi file
            ...
            if (move_uploaded_file($foto['tmp_name'],$dir.$newName)) {
               ...
               $sql = $con->prepare("INSERT INTO pemenang(nama_calon, alamat, foto, visi, judull, periode) VALUES(?, ?, ?, ?, ?, ?)") or die($con->error);
               $sql->bind_param('ssssss', $nama, $alamat, $newName, $visi, $judull, $periode);
               $sql->execute();
